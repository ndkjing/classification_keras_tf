...
def preprocess_input(x, data_format=None):
    ...
    # Arguments
        x: 4D numpy input
        data_format: insight_data format of the image tensor.
    # Returns
        Preprocessed tensor.
    """
...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
    ...
    Args:
        x: keras tensor
        nb_layers: the number of layers of conv_block to append to the weights.
        nb_filter: number of filters
...
def get_secdnsenet121():
    model =SEDenseNetImageNet121()
    return model
