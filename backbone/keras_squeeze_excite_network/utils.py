...
def _obtain_input_shape(input_shape,
                        ...
                        require_flatten,
                        weights=None):
    """Internal utility to compute/validate a weights's tensor shape.
    # Arguments
        input_shape: Either None (will return the default network input shape),
            or a user-provided shape to be validated.
        default_size: Default input width/height for the weights.
        min_size: Minimum input width/height accepted by the weights.
        data_format: Image insight_data format to use.
        require_flatten: Whether the weights is expected to
    ...
    if weights != 'imagenet' and input_shape and len(input_shape) == 3:
        if data_format == 'channels_first':
            if input_shape[0] not in {1, 3}:
                warnings.warn(
                    'This weights usually expects 1 or 3 input channels. '
