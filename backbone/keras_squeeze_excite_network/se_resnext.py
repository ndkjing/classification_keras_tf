...
def SEResNext(input_shape=None,
    ...
    """Instantiate the ResNeXt architecture. Note that ,
        ...
        at ~/.keras/keras.json.
        The weights are compatible with both
        TensorFlow and Theano. The dimension ordering
        convention used by the weights is the one
        specified in your Keras config file.
        # Arguments
            depth: number or layers in the ResNeXt weights. Can be an
...
def __create_res_next(nb_classes, img_input, include_top, depth=29, cardinality=8, width=4,
    ...
    """ Creates a ResNeXt weights with specified parameters
    Args:
...
def get_seinceptionv3(classes=10):
    model = SEResNext(classes=classes)
    return model


if __name__ == '__main__':
    model = SEResNext(classes=10)
    model.summary()
