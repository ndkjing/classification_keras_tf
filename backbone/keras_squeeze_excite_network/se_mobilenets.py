...
def SEMobileNet(input_shape=None,
    ...
    therefore it only works with the insight_data format
    `image_data_format='channels_last'` in your Keras config
    at `~/.keras/keras.json`.
    To load a MobileNet weights via `load_model`, import the custom
    objects `relu6` and `DepthwiseConv2D` and pass them to the
    `custom_objects` parameter.
    E.g.
    weights = load_model('mobilenet.h5', custom_objects={
                       'relu6': mobilenet.relu6,
                       'DepthwiseConv2D': mobilenet.DepthwiseConv2D})
    # Arguments
        input_shape: optional shape tuple, only to be specified
            if `include_top` is False (otherwise the input shape
            has to be `(224, 224, 3)` (with `channels_last` insight_data format)
            or (3, 224, 224) (with `channels_first` insight_data format).
        ...
        pooling: Optional pooling mode for feature extraction
            ...
            - `None` means that the output of the weights
                will be the 4D tensor output of the
                last convolutional layer.
            - `avg` means that global average pooling
                will be applied to the output of the
                last convolutional layer, and thus
                the output of the weights will be a
                input shape
    ...
    # Returns
        A Keras weights instance.
    # Raises
        ValueError: in case of invalid argument for `weights`,
            or invalid input shape.
        RuntimeError: If attempting to run this weights with a
...
def _conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1)):
    ...
    # Arguments
        inputs: Tensor of shape `(rows, cols, 3)`
            (with `channels_last` insight_data format) or
            (3, rows, cols) (with `channels_first` insight_data format).
            It should have exactly 3 inputs channels,
            and width and height should be no smaller than 32.
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    # Arguments
        inputs: Tensor of shape `(rows, cols, channels)`
            (with `channels_last` insight_data format) or
            (channels, rows, cols) (with `channels_first` insight_data format).
